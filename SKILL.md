...
description: "当用户提供一个或多个 Excel/CSV 表格，要求进行业务数据分析，并将分析过程、计算结果、可视化看板和图文分析报告发送沉淀到飞书中时使用。该技能会创建飞书多维表格作为数据分析过程记录仓库，按原子步骤写入数据表和结果表，建立飞书 Base 仪表盘，并创建飞书云文档报告引用对应的多维表格产物。"
metadata:
  requires:
    bins: ["lark-cli"]
