...
> **前置条件：** 先阅读 `auth-and-permission.md` 了解认证、全局参数和安全规则。
...
| 在仪表盘里添加组件 | `+dashboard-block-create` | 先读 [lark-base-dashboard-block-create.md](lark-base-dashboard-block-create.md)，再读 `dashboard-block-data-config.md` |
| 修改组件 | `+dashboard-block-update` | 先读 [lark-base-dashboard-block-update.md](lark-base-dashboard-block-update.md)，再读 `dashboard-block-data-config.md` |
...
详细组件类型和 data_config 完整规则：`dashboard-block-data-config.md`
...
2. 再读 `dashboard-block-data-config.md` 了解：
   - 全部组件类型的可复制模板
   - filter 筛选条件格式
   - 字段类型与操作符对应表
...
2. 再读 `dashboard-block-data-config.md` 了解 data_config 结构
